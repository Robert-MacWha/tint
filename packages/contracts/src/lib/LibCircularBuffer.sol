...
library LibCircularBuffer {
    ...
    function requireSpace(CircularBuffer storage self, uint128 count) internal view {
        if (space(self) < count) {
    ...
    function space(CircularBuffer storage self) internal view returns (uint128) {
    ...
    function get(CircularBuffer storage self, uint128 _index) internal view returns (bytes32) {
        if (_index >= self.head) {
    ...
    function requireAdvancable(CircularBuffer storage self, uint128 newTail) internal view {
    ...
    function advanceTail(CircularBuffer storage self, uint128 newTail) internal {
        requireAdvancable(self, newTail);
        self.tail = newTail;
    }

    /// @notice Returns x as an index in the circular buffer, wrapping if necessary.
    function index(CircularBuffer storage self, uint128 x) private view returns (uint128) {
