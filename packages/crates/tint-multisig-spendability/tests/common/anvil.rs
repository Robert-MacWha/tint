...
use crate::common::anvil::{
    ...
    spendability::MultisigSpendability::MultisigSpendabilityInstance,
};
...
pub struct Instance {
    ...
    pub spendability: MultisigSpendabilityInstance<DynProvider>,
}
...
pub mod spendability {
    ...
    sol!(
        #[sol(rpc)]
        MultisigSpendabilityVerifier,
        "../../contracts/out/MultisigSpendabilityVerifier.sol/Verifier.json"
    );

    sol!(
        #[sol(rpc)]
        MultisigSpendability,
        "../../contracts/out/MultisigSpendability.sol/MultisigSpendability.json"
    );
}

/// Sets up an anvil instance for testing, deploying Tint, a mock ERC20, and
/// the MultisigSpendability contracts.
#[allow(dead_code)]
pub async fn setup() -> anyhow::Result<Instance> {
    ...
    let spendability_verifier =
        spendability::MultisigSpendabilityVerifier::deploy(provider.clone()).await?;
    let spendability = spendability::MultisigSpendability::deploy(
        provider.clone(),
